...
| SDK levels | `minSdk` = `targetSdk` = 35 (Android 15); `compileSdk` = 37 | Personal, sideloaded app on a single device (Android 17), so no reason to support old versions. Removes `Build.VERSION` branches and the AppCompat per-app-language backport (the platform `LocaleManager` is available from 33). `targetSdk` 35 keeps runtime behavior at a level we know well. `compileSdk` is raised only because current AndroidX libraries require it; it affects compilation, not runtime behavior | `minSdk` 26 (extra compat code for no benefit); `targetSdk` 37 (newest behavior changes are less familiar); `compileSdk` 35 (would mean pinning stale AndroidX versions) |
| Build tooling | Gradle 9.7.1 (checksum-pinned wrapper), AGP 9.x with built-in Kotlin, JDK 21 for running Gradle, Java 17 target | Current stable toolchain. AGP 9 needs Gradle 9.6+ and JDK 17+. The Android Studio bundled JDK 25 is too new for Gradle 8.x, so the IDE's Gradle JDK is set to 21 | Staying on the wizard's AGP 8.9 / Gradle 8.11 (stale) |
